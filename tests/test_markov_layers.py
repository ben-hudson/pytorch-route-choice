...
def test_values_and_probs(small_network, linear_solver):
    ...
    exp_values, info = linear_solver(
        torch_graph.edge_index, exp_rewards, sink_node_mask, sink_node_mask.clone()
    )
...
def test_flows(rl_tutorial_network, linear_solver):
    ...
    exp_values, values_info = linear_solver(
        torch_graph.edge_index, exp_rewards, sink_node_mask, sink_node_mask.clone()
    )
    values = exp_values.log()
    probs = edge_prob(torch_graph.edge_index, exp_rewards, exp_values, sink_node_mask)
    ...
    node_flows, flows_info = linear_solver(torch_graph.edge_index.flip(0), probs, demand, demand.clone())
