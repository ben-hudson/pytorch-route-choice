...
def test_values_and_probs(small_network):
    ...
    model = RecursiveLogitRouteChoice(None, node_dim=-1, f_solver="fixed_point_iter", f_tol=1e-5)

    rewards = -torch_graph.cost.unsqueeze(0)
    sink_node_mask = torch_graph.is_dest.unsqueeze(0)
    values, probs = model.get_values_and_probs(torch_graph.edge_index, rewards.exp(), sink_node_mask)
